...
app.put(
  "/posts/:id",
  async (
    /* Req has a generic body that goes like this
        Request<Params = any, ResBody = any, ReqBody = any, ReqQuery = any>
        I specify the type of the params and the reqBody using my interface type
    */
...
app.delete("/posts/:id", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.status(200).json({ mesage: `Post ${id} deleted successfully` });
  } catch (err) {
    if (err instanceof Error) {
      res.status(500).json({ error: err.message });
    } else {
